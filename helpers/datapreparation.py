...
def load_all_dataset(dirpath, binarize=True, first_column=1, remove_padding=False):
    ...
    if(binarize):
        dataset = [(pd.read_csv(os.path.join(dirpath, file)).values>0).astype(int) for file in sorted(os.listdir(dirpath)) if file.endswith(".csv")]
    else:
        dataset = [(pd.read_csv(os.path.join(dirpath, file)).values) for file in sorted(os.listdir(dirpath)) if file.endswith(".csv")]

    if first_column > 0:  #With first_column=1, skips the key_indices
        dataset = [data[:, first_column:] for data in dataset]
    ...
    if remove_padding:
        ...
        for data in dataset:
            for row, i in zip(data, range(len(data))):
                if i < min_index and np.any(row):
                    min_index = i
                
                if i > max_index and np.any(row):
                    max_index = i

                if i >= min_index and len(data) <= max_index:
                    break

        dataset = [data[min_index:max_index+1] for data in dataset]
...
def generate_round(model,tag,n,k=1,init=None):
    if(init is None):
        ...
        k = init.shape[0]
    res = init
    model.init_hidden(tag)
    for i in range(n//k):
        init = model.forward(init)
